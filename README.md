...
Built a custom onboarding experience using "Paper-onboarding" that permits the user to get an overall feel of the app. 
...
# Rerouting
Using the Google Maps SDK, we created a rerouting feature that permits the user to press down on the spot they want to go through. This adds a new waypoint which sends the route through the waypoint. 

<img src="https://github.com/C4Q/AC3.2-Navty/blob/master/Navty/Demo/Navty_Navigating_Demo.gif?raw=true" width="320" />

# Tracking & Website

Tracking can be enabled in the menu sidebar by switching the tracking functionality to on. When tracking is on and your destination is entered, a text message is presented with a pre-populated message that contains a URL to the Navty Website which can be sent to your emergency contacts. The Website can be used to track the user if you do not have the app to track the user.  
